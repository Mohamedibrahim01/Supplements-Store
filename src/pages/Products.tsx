    ...
    <div className="min-h-screen p-10 pt-24 bg-gradient-to-br from-[#98ff98] via-white to-[#ccffcc] dark:from-green-950 dark:via-green-900 dark:to-black transition-colors duration-300">
      ...
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {ProductsData.map((product) => (
          <motion.div
            key={product.id}
            className="bg-white dark:bg-green-950 rounded-2xl shadow-md border border-green-200 dark:border-green-700 transition-all duration-300"
            ...
            <Link to={`/products/${product.id}`}>
              <div className="relative">
                <img
                  src={product.image}
                  alt={product.name}
                  className="w-full h-64 object-cover rounded-t-2xl brightness-105 contrast-110"
                />
